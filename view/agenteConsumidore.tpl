{extends file='pagina.tpl'} {block name='pagina'}
<div class='title'>Torne-se um agente Moneyone</div>
<div class='subtitle'>Quer complementar sua renda e ainda oferecer mais um serviço em seu estabelecimento?</div>
<p>Devido a uma alteração nas legislações europeia e portuguesa, você pode agora oferecer o serviço de transferência de fundos da MoneyOne a partir do seu estabelecimento, tornando-se assim um agente MoneyOne.</p>
<p>Ao oferecer o serviço, você tem uma oportunidade única de aproveitar a significativa procura já existente de transferências de fundos.</p>
<div class='subtitle'>Mais informações</div>
<ul>
	<li><a href="agenteAgente.php" >Benefícios para agente</a></li>
	<li><a href="agenteConsumidore.php" >Benefícios para os consumidores</a></li>
	<li><a href="agenteTornarAgente.php" >Como tornar-se um Agente MoneyOne</a></li>
	<li><a href="agenteFAQ.php" >Perguntas frequentes</a></li>
</ul>
<div class='title'>Benefícios para os consumidores</div>
<p>Ao tornar-se um agente MoneyOne, você poderá oferecer os seguintes benefícios aos seus clientes:</p>
<ul>
	<li>Transferências de fundos rápidas, confiáveis e sem taxas de envio.</li>
	<li>Uma rede abrangente Com o serviço MoneyOne seus clientes poderão enviar fundos para mais de 100 países em todo o mundo.</li>
	<li>Programa de fidelização Com o Cartão de Pontos MoneyOne, seus clientes poupam tempo e recebem pontos por cada transacção realizada. <a href="cartaoPontos.php" >Descubra mais sobre o programa.</a>
	</li>
</ul>
{/block}
